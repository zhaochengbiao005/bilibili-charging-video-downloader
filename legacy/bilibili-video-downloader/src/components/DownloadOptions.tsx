...
import { Settings2, Film, Music, Download, AlertTriangle } from 'lucide-react';
...
export function DownloadOptions({
  ...
  return (
    <div className="glass-panel rounded-[2rem] p-6 lg:p-8 flex flex-col h-full shrink-0">
      <div className="flex items-center gap-3 mb-6">
        <div className="text-bili-pink flex items-center justify-center">
          <Settings2 size={24} strokeWidth={2.5} />
        </div>
        <h3 className="text-xl font-bold text-gray-900">下载设置</h3>
      </div>

      {/* 格式选择 */}
      <div className="mb-5">
        <label className="block text-sm font-bold text-gray-600 mb-3">格式</label>
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <span className="font-bold text-gray-800 flex items-center gap-2">
                    {q}
                    {isPremium && <span className="text-[10px] bg-yellow-100 text-yellow-600 px-2 py-0.5 rounded-full">PRO</span>}
                  </span>
                </div>
