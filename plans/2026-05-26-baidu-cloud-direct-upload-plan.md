...
- 弹幕在云盘模式下支持两种方式：外挂 `.ass` 小文件上传，或通过 FFmpeg 管道烧录进云端直传 MP4；烧录过程只写入小型 ASS 临时文件，不写本地大视频文件。
...
- 烧录弹幕会增加 CPU、内存和失败恢复复杂度，因此通过 FFmpeg stdout 双遍管道实现，并在第二遍上传前校验分片 MD5 是否与上传计划一致。
...
- [x] 第二遍重复输出并分片上传百度网盘。
- [x] UI 标记为 MP4 云端直传。
- [x] 支持云盘 MP4 管道直传时烧录弹幕。
- [x] 烧录弹幕只写小型 ASS 临时文件，不写本地大视频文件。
- [x] 第二遍 FFmpeg 输出上传前校验分片 MD5，避免两遍输出不一致时继续上传。
